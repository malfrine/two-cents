...
class LinearProgramStrategy(AllocationStrategy):
    def create_solution(self, problem_input: UserPersonalFinances) -> FinancialPlan:
        ...
        instruments_and_times = list(
            (i.name, t) for i in instruments for t in range(max_end_date)
        )
        ...
        X = problem_input.financial_profile.monthly_allowance
        ...
        m.c1 = pe.Constraint(loans_and_times, rule=min_payments_loans)
        m.c2 = pe.Constraint(instruments_and_times, rule=money_balance)
        m.c3 = pe.Constraint(time_periods, rule=payment_sum)
        m.c4 = pe.Constraint(loans_and_times, rule=lambda m, i, t: m.v[i, t] <= 0)
        m.c5 = pe.Constraint(
            list(
                (i, t) for i in loan_names for t in range(end_date[i] - 1, max_end_date)
            ),
            rule=lambda m, i, t: m.v[i, t] == 0,
        )
        m.c6 = pe.Constraint(
            loans_and_times, rule=lambda m, i, t: m.v[i, t] >= -10000000000 * m.b[i, t]
        )
        ...
        print(results)
        if not (
            (results.solver.status == pe.SolverStatus.ok)
            and (
        ...
        return _make_solution(
            problem_input.portfolio,
            monthly_payments,
            problem_input.financial_profile.monthly_allowance,
        )
